...
支持文件夹批量输入和指定输出目录。批量模式读取指定文件夹第一层的常见视频文件，为每个视频建立可独立暂停、取消和查看日志的任务；外部输出目录中的产物不会随任务记录删除。

翻译会明确保留成人影视语境，同时避免无依据地色情化普通对白；对俗语、双关、数数歌等文化表达进行自然中文转换。双 Whisper 识别结果会作为翻译与最终校正的参考，识别冲突时优先标记不确定而不是编造对白。
