...
重点消除字典式直译。人物调侃、夸赞、双关、俗语、儿歌、数数歌和文化梗，要结合前后文改成中国观众自然理解的表达，不能留下表面逐词对应但中文不成立的句子。
asr_review 是两种识别模型的对照；若 disagreement 为 true 且两个版本差异明显，只能在上下文和声音证据足够时修正，否则保留不确定性，绝不能编造对白。
            ...
            context.append(
                {
                    ...
                    **({"asr_review": current["asr_review"]} if current.get("asr_review") else {}),
                }
            )
        request = {
