...
function cloudWorkerBody(){return{enabled:$('#cloud-worker-enabled').checked,host:$('#cloud-worker-host').value.trim(),port:Number($('#cloud-worker-port').value||22),username:$('#cloud-worker-username').value.trim(),password:$('#cloud-worker-password').value,private_key_path:$('#cloud-worker-key').value.trim(),remote_dir:$('#cloud-worker-dir').value.trim(),model_dir:$('#cloud-worker-model-dir').value.trim(),auto_setup:$('#cloud-worker-auto-setup').checked}}
...
async function init(){
  ...
  const worker=saved?.cloud_worker||{};$('#cloud-worker-enabled').checked=runtimeInfo.mode==='local'&&Boolean(worker.enabled||asr.kind==='accuracy_ensemble');$('#cloud-worker-host').value=worker.host||'';$('#cloud-worker-port').value=worker.port||22;$('#cloud-worker-username').value=worker.username||'root';$('#cloud-worker-password').value=worker.password||'';$('#cloud-worker-key').value=worker.private_key_path||'';$('#cloud-worker-dir').value=worker.remote_dir||'/root/subtitle-worker';$('#cloud-worker-model-dir').value=worker.model_dir||'/root/autodl-tmp/subtitle-models';$('#cloud-worker-auto-setup').checked=worker.auto_setup!==false;if(worker.password_configured&&!worker.password)$('#cloud-worker-password').placeholder='已加密保存';
...
$('#resume-all-jobs').onclick=async()=>{const buttons=$$('.job-bulk-actions button');buttons.forEach(button=>button.disabled=true);try{const soft=$('#soft-video').checked,hard=$('#hard-video').checked;const data=await jsonFetch('/api/jobs/actions/resume-all',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({create_soft_subtitle_video:soft,create_hard_subtitle_video:hard})});const failed=data.failed?.length||0;alert(`全部继续：成功 ${data.count} 个${failed?`，另有 ${failed} 个未处理`:''}。已应用当前产物设置：软字幕视频 ${soft?'开启':'关闭'}，硬字幕视频 ${hard?'开启':'关闭'}`);await renderJobs()}catch(e){alert(`全部继续失败：${e.message}`)}finally{buttons.forEach(button=>button.disabled=false)}};
$('#start-all-staged-jobs').onclick=()=>bulkJobAction('start-staged-all',{label:'开始全部已预上传任务'});
$('#retry-failed-staged-jobs').onclick=()=>bulkJobAction('retry-stage-failed',{label:'重试失败预上传'});
...
function actionMarkup(j){if(['queued','running'].includes(j.status))return`<button onclick="pauseJob('${j.id}')">暂停</button><button class="danger" onclick="cancelJob('${j.id}')">取消</button>`;if(j.status==='paused')return`<button onclick="resumeJob('${j.id}')">继续</button><button class="danger" onclick="cancelJob('${j.id}')">取消</button>`;if(j.status==='staged')return`<button onclick="startStagedJob('${j.id}')">GPU 开机后开始处理</button><button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`;if(j.status==='failed'&&j.options.cloud_stage_only)return`<button onclick="retryStagedJob('${j.id}')">断点续传</button><button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`;if(j.status==='failed')return`<button onclick="retryFailedJob('${j.id}')">从失败处继续</button><button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`;return`<button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`}
