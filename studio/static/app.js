...
async function init(){
  ...
  const worker=saved?.cloud_worker||{};$('#cloud-worker-enabled').checked=Boolean(worker.enabled);$('#cloud-worker-host').value=worker.host||'';$('#cloud-worker-port').value=worker.port||22;$('#cloud-worker-username').value=worker.username||'root';$('#cloud-worker-password').value=worker.password||'';$('#cloud-worker-key').value=worker.private_key_path||'';$('#cloud-worker-dir').value=worker.remote_dir||'/root/subtitle-worker';$('#cloud-worker-model-dir').value=worker.model_dir||'/root/autodl-tmp/subtitle-models';$('#cloud-worker-auto-setup').checked=worker.auto_setup!==false;if(worker.password_configured&&!worker.password)$('#cloud-worker-password').placeholder='已加密保存';syncCloudWorkerFields();
  fillSavedModel('#asr-model',asr.kind==='local_whisper'?localWhisperModels:(asr.kind==='accuracy_ensemble'?['accuracy-ensemble-v1']:(asr.model?[asr.model]:[])),asr.kind==='local_whisper'?installedWhisperModels:null,asr.kind==='accuracy_ensemble'?'accuracy-ensemble-v1':(asr.model||'medium'),asr.kind!=='accuracy_ensemble');
...
function syncProviderFields(){
  ...
  if(ak!==lastAsrKind){
    ...
    else if(ak==='accuracy_ensemble')fillModelSelect('#asr-model',['accuracy-ensemble-v1'],null,'accuracy-ensemble-v1',false);
